...
const { sendVerificationOtpEmail } = require('./config/mailer');
    ...
    if (existing) {
      if (!existing.isEmailVerified) {
        return res.status(400).json({
          error: 'Account exists but email is not verified. Please verify your email or resend OTP.'
        });
      }
...
app.post('/api/verify-login-otp', async (req, res) => {
  try {
    ...
    if (!user.loginOtpHash || !user.loginOtpExpiresAt) {
      return res.status(400).json({ error: 'No active login OTP found. Please login again.' });
    }

    if (new Date() > new Date(user.loginOtpExpiresAt)) {
      return res.status(400).json({ error: 'Login OTP expired. Please login again.' });
    }
...
// Resend login OTP
app.post('/api/resend-login-otp', async (req, res) => {
  try {
    const { email } = req.body;
    if (!email) {
      return res.status(400).json({ error: 'Email is required' });
    ...
    if (!user.isEmailVerified) {
      return res.status(400).json({ error: 'Email is not verified yet. Verify email first.' });
    }
